...
interface AcceptanceResponse {
  data?: {
    presigned_acceptance?: AcceptanceItem;
    presigned_personal_data_auth?: AcceptanceItem;
  };
}

export interface PublicAcceptanceLinks {
  terms: string;
  personalData: string;
}
...
function hasAcceptanceTokens(data: AcceptanceResponse["data"]): data is NonNullable<AcceptanceResponse["data"]> & {
  presigned_acceptance: AcceptanceItem;
  presigned_personal_data_auth: AcceptanceItem;
} {
  return Boolean(
...
export async function getAcceptanceTokens() {
  ...
  if (!response.ok || !hasAcceptanceTokens(payload.data)) {
    throw new Error("Wompi no devolvió los contratos de aceptación.");
  }
...
export function transactionForBrowser(transaction: WompiTransaction) {
  return {
    ...
    processor_response_code:
      transaction.payment_method?.extra?.processor_response_code ??
      transaction.payment_method?.extra?.return_code ??
      null
  };
}
