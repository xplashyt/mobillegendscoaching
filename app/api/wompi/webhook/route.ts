...
import { isNexoCincoReference, parseReference } from "@/lib/orders";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validChecksum(event, request.headers.get("x-event-checksum"), config.eventsSecret)) {
      return NextResponse.json({ received: false }, { status: 401 });
    }

    const expectedEnvironment = config.environment === "test" ? "test" : "prod";
    if (event.environment && !event.environment.toLowerCase().includes(expectedEnvironment)) {
      return NextResponse.json({ received: true });
    }

    const transaction = event.data?.transaction;
    if (!transaction || typeof transaction.reference !== "string" || !isNexoCincoReference(transaction.reference)) {
      return NextResponse.json({ received: true });
    }
    ...
    const matchesOrder =
      order &&
      transaction.amount_in_cents === order.plan.priceCOP * 100 &&
