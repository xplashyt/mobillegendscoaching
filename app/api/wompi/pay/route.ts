...
export async function POST(request: NextRequest) {
  ...
  try {
    input = (await request.json()) as PayRequest;
  } catch {
    return NextResponse.json({ message: "La solicitud de pago no es válida." }, { status: 400 });
  }
  ...
  if (!input.acceptedTerms) {
    return NextResponse.json({ message: "Debes aceptar los contratos de Wompi." }, { status: 400 });
  }
  if (!input.token?.startsWith("tok_") || !parsed) {
    return NextResponse.json({ message: "El plan o el token de pago no es válido." }, { status: 400 });
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || fullName.length < 3) {
    return NextResponse.json({ message: "Revisa el correo y el nombre del titular." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!result.ok || !result.payload.data) {
      ...
      return NextResponse.json(
        {
          message: details[0] ?? "Wompi no pudo crear la transacción.",
          details
        },
        { status: result.statusCode >= 400 && result.statusCode < 500 ? 422 : 502 }
      );
    }
